...
#    if k!=26: continue;
    ...
    H = DirichletGroup(N, base_ring=CyclotomicField(1));
...
#            gfs = (sqrt(1.0*N)/2/pi)^s * factorial(s-1) * lse(s);
#            gfks = (sqrt(1.0*N)/2/pi)^(k-s) * factorial(k-s-1) * lse(k-s);
#            print("epsilon_f:", gfs/gfks);
        ...
        zd.append([k,zu,zl])
        with open('rfzs.txt', 'a') as fd:
            c = csv.writer(fd)
            c.writerows([zz]);
        with open('rfzs_unit_circle.txt', 'a') as fd:
            c = csv.writer(fd)
            c.writerows([zza])
#print(zd)
