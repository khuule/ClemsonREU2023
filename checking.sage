...
#4-d Array containing all roots in the format [N-1][M-1][eps(f)][ith newform]
ZZs = load_roots(NN,MM);
#Check every N in range
for N in range (1,NN+1):
    #Check every M up to MM
    for M in range(0, 39):
        for Mp in range (M,len(ZZs[N-1])):
                        ...
                        for y in range(0, len(ZZs[N-1][Mp][sp])):
                            ...
                            for l in range(0, len(z)-1):
                                ...
                                if (s==sp and (z[0]==M or z[len(z)-1]==M)):
                                    stieltjes = False;
                                #Strong Stieltjes diff sign(includes interlacing, same weight)
